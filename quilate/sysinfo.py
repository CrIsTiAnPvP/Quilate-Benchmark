...
# La salud de los discos vive en `smart`: el inventario dice qué hay, y eso dice
# cómo está. Se reexporta lo que el inventario usa para armar cada volumen.
from .smart import _smart_del_disco, _smart_por_modelo, _storage_reliability
...
def _collect_windows_info(si: SystemInfo) -> None:
    """El inventario de Windows, volcado en `si` pieza por pieza.

    Lo que se puede leer sin permisos y lo que no llegan por caminos distintos y
    se juntan aqui. Cada clave conserva su propio `ok`, asi que un lote elevado
    que no se llego a pedir se distingue de uno que se pidio y fallo.

    El reparto en `_volcar_*` no es cosmetico: cada bloque tiene sus propias
    trampas —el entero de 32 bits de la VRAM, la velocidad nominal de la RAM
    frente a la real— y con los seis seguidos en una sola funcion habia que leer
    cien lineas para cambiar cuatro.
    """
    inventario = _inventario_windows()
    inventario.update(elevacion.recoger())
    _volcar_os(si, inventario["os"])
    _volcar_cpu(si, inventario["cpu"])
    _volcar_memoria(si, inventario["mem"])
    _volcar_gpus(si, inventario["gpus"])
    _map_storage(si, inventario)
    _volcar_bios(si, inventario["bios"])
    _volcar_chasis(si, inventario["chassis"])


def _volcar_os(si: SystemInfo, osinfo) -> None:
    if osinfo:
        d = osinfo[0]
...
def _volcar_gpus(si: SystemInfo, gpus) -> None:
